<template>
  <Module class="flex-grow h-1/3" :info="bsMeta && bsMeta['emplRate']" :loading="loading" :error="error">
    <template #title>里程排行榜(公里)</template>
    <!-- <template #end>
      <Switcher :options="options" v-model="latest" />
    </template> -->
    <template v-if="!loading">
      <div style="width: 100%">
        <div v-for="(item, index) in roadData" :key="index" class="jcsstj-outline flex justify-between items-center" style="margin-top: 5px">
          ...
          <div class="jcsstj-blout" :class="index < 3 ? 'jcsstj-blout-top' : ''">
            <div class="jcsstj-bl" :class="index < 3 ? 'jcsstj-bl-top' : ''" :style="{ width: item.sectionLength/roadTotal*100 + '%' }"></div>
            <div class="bl-hightlinght" :style="{ left: item.sectionLength/roadTotal*100 + '%' }" :class="index < 3 ? 'bl-hightlinght-top' : ''"></div>
          </div>
          <div class="jcsstj-cont">{{ item.sectionLength }}</div>
        </div>
      </div>
      <!-- <Ect
        :options="ectOpts"
...
const roadTotal = computed(() => {
  return roadData.value.map((e: any) => Number(e['sectionLength']))?.reduce((a: number, b: number) => a + b) || 1;
});
