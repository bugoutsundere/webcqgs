<template>
  <Module class="h-1/3" :loading="loading" :error="error" :info="bsMeta && bsMeta['utilities']">
    <template #title>基础设施统计(数量)</template>
    <div class="base-struct-list" v-if="arr">
      <div class="list-item-box" :class="{ single: !item.children }" v-for="item in baseStructInfo" :key="item.code">
        <div class="list-item" @click="activeBaseStruct = item.code" :class="{ active: activeBaseStruct === item.code }" v-waves>
          <div class="item-icon">
            <img v-if="activeBaseStruct === item.code" :src="imgslist.value[item.code + 'Act']" />
          ...
          <template v-else>
            ...
            <ul class="item-subtypes">
              <li class="item-subtype" v-for="subItem in childrenData[item.key]" :key="subItem.key">
                ...
                <span class="sub-num">{{formatThousand(subItem.count)}}</span>
              </li>
            </ul>
          </template>
        </div>
