<template>
  <div
    class="eq-list-win animate__animated"
    :class="{
  ...
  >
    <Module
      class="module w-ful h-full"
      theme="none"
      :info="bsMeta && bsMeta['machineSubtype']"
      :loading="loading"
      :error="error"
    >
      ...
      <div class="absolute inset-0 flex" v-if="dataList?.length > 0 && !loading">
        <div class="w-full relative flex">
          <el-table size="small" class="flex-grow" :data="dataList" height="100%">
            <el-table-column align="center" prop="typeName" label="设备类型"></el-table-column>
            <el-table-column align="center" prop="eqName" label="设备名称"></el-table-column>
            <el-table-column align="center" label="设备数量">
              <template #default="scope">
                <span :style="{ fontSize: '14px', fontWeight: 500 }">{{ scope.row.eqNums }}</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
