<template>
  <Module class="w-1/3" :loading="loading" :error="error" :info="bsMeta && bsMeta['machine']">
    <template #title>机电设备统计(数量)</template>
    <template #end>
      <button class="endButton" @click="clickEndButton">机电设备详情</button>
    </template>
    <ul class="eq-list" v-if="arr">
      <li class="eq-item-box" v-for="eq in eqInfo" :key="eq.key">
        <div
          class="eq-item"
          :class="{ active: activeEq === eq.code }"
          @click="activeEq = eq.code"
          v-waves
        >
          <svg-icon class="icon" :name="'eq-'+eq.key" />
          <span class="eq-value">
            {{ arr ? formatThousand(arr[eq.code] || 0) : '-' }}
          </span>
          <span class="eq-name mt-1">{{ eq.name }}</span>
        </div>
      </li>
    </ul>
...
const { data, loading, error, refresh } = useRequest<any>(() =>
  genConfig(
    `/http-server-yonyu/publish/jsc/eqNums${
